...
if __name__ == '__main__' :
  ...
  for infile in infiles :
    ...
    #= trim adapters
      if not adapter == 'none':
        input_rdd = input_rdd.map(lambda e: trim_adapter (e, adapter))
    ...
    ## in : ('seq', freq)
    ## out: ('seq', freq)
    RDD = collapse_rdd.filter(lambda e: int(e[1]) > limit_srna_freq)
    
    #= Filtering short length
    ## in : ('seq', freq)
    ## out: ('seq', freq)
    sr_short_rdd = RDD.filter(lambda e: len(e[0]) > limit_len).persist()  # TO KEEP IT

    #'''
    #= Filtering with DustMasker
    ## in : ('seq', freq)
    ## out: 'seq'
    RDD = sr_short_rdd.map(lambda e: '>s\n'+e[0])\
    ...
    bowFrq_rdd = RDD.join(sr_short_rdd)\
    ...
    ## out: ('seq', [freq, nbLoc, [['strd','chr',posChr],..]])
    RDD = bowFrq_rdd.filter(lambda e: e[1][0] > limit_mrna_freq)
    
    #= Filtering high nbLocations and zero location
    ## in : ('seq', [freq, nbLoc, [['strd','chr',posChr],..]])
    ## out: ('seq', [freq, nbLoc, [['strd','chr',posChr],..]])
    RDD = RDD.filter(lambda e: e[1][1] > 0 and e[1][1] < limit_nbLoc)
    
    #= Flatmap the RDD
    ## in : ('seq', [freq, nbLoc, [['strd','chr',posChr],..]])
    ## out: ('seq', [freq, nbLoc, ['strd','chr',posChr])
    RDD = RDD.flatMap(mru.flatmap_mappings)
    ...
    RDD = RDD.map(lambda e: mircheck_obj.mirCheck_map_rule(e, 3))\
    ...
    ## out: ('seq', [freq, nbLoc, ['strd','chr',posChr], ['priSeq',posMirPri,'priFold','mkPred','mkStart','mkStop']])
    RDD = RDD.filter(lambda e: ut.containsOnlyOneLoop(e[1][3][2][int(e[1][3][4]) : int(e[1][3][5])+1]))
    ...
    RDD = RDD.map(lambda e: prec_obj.extract_prem_rule(e, 3))
    ...
    RDD = RDD.map(lambda e: rnafold_obj.RNAfold_map_rule(e, 4))
    ...
    RDD = RDD.map(lambda e: profile_obj.computeProfileFrq(e, dict_bowtie_chromo_strand))\
                      .filter(lambda e: e[1][0] / float(e[1][5]) > 0.2)

    print('NB final prediction distinct: ', len(RDD.groupByKey().collect()))
